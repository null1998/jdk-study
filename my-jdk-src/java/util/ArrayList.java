 ...
 * set,isEmpty,get,set,iterator和ListIterator操作在常数时间内运行。
 ...
 * time in the future.
 * 此类的返回的迭代器iterator和listIterator方法是快速失败的：
 * 如果List在创建迭代器之后的任何时间内，以任何方式，除了通过迭代器自身的remove或add方法，
 ...
 * 请注意，迭代器的快速失败行为不能被保证
 * 因为通常来说，不可能在交易中做出任何硬性保证，
 * 保证存在不同步的并发修改。快速失败迭代器
